...
//            e.printStackTrace();
    ...
    /**
     * 结束所有Activity 除了当前的actvity
     */
    public void removeAllExcuCurrentActivity() {
        for (int i = 0, size = activityStack.size(); i < size; i++) {
            if (null != activityStack.get(i) && !activityStack.get(i).getClass().getName().equals(getAppManager().currentActivity().getClass())) {
                activityStack.get(i).finish();
            }
        }
        activityStack.clear();
...
//                }
    ...
    public void AppExit(Context context, Boolean isBackground) {
        try {
            ...
            GreenDaoManager.getInstance().closeUserConnection();
